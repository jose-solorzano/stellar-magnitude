...
from models.BandBasedMagnitudeModel import BandBasedMagnitudeModel
from models.DistanceBasedMagnitudeModel import DistanceBasedMagnitudeModel


class BandBasedMagnitudeModelTrainer:
    def __init__(self, num_passes: int, num_splits: int, max_training_items: int, num_epochs: int, lr: float,
                 id_column='source_id', response_column='response', target_column='allwise_w3'):
    ...
    def get_inputs(self, data_frame: pd.DataFrame):
        ...
        band_magnitudes = data_frame[self.mag_columns].values
        band_magnitudes_t = torch.from_numpy(band_magnitudes).float()
        return band_magnitudes_t, distance_t, color_metrics_t, lat_rad_t, long_rad_t,
    ...
    def train_fold(self, train_frame: pd.DataFrame, est_band_params: np.ndarray):
        band_magnitudes_t, distance_t, color_metrics_t, lat_rad_t, long_rad_t = self.get_inputs(train_frame)
        target_mag_t = self.get_label(train_frame)
        est_band_params_t = torch.from_numpy(est_band_params).float()
        num_bands = len(est_band_params)
        model = BandBasedMagnitudeModel(est_band_params_t, num_bands)
        model.train()
        loss_fn = nn.MSELoss()
        optimizer = optim.Adam(model.parameters(), lr=self.lr)
        print('Training split...')
        for epoch in range(self.num_epochs):
            pred_mag = model(band_magnitudes_t, distance_t, color_metrics_t, lat_rad_t, long_rad_t)
    ...
    def get_response_frame(self, data_frame: pd.DataFrame, model: BandBasedMagnitudeModel):
        band_magnitudes_t, distance_t, color_metrics_t, lat_rad_t, long_rad_t = self.get_inputs(data_frame)
        model.eval()
        pred_mag = model(band_magnitudes_t, distance_t, color_metrics_t, lat_rad_t, long_rad_t)
    ...
    def train_pass(self, pass_number: int, data_frame: pd.DataFrame, est_band_params: np.ndarray):
        ...
        for train_idx, test_idx in kf.split(shuffled_frame):
            ...
            fold_model = self.train_fold(train_frame, est_band_params)
    ...
    def train(self, data_frame: pd.DataFrame):
        linear_params = self.estimate_band_params(data_frame)
    ...
    def estimate_band_params(self, data_frame: pd.DataFrame):
        band_vars = data_frame[self.mag_columns].values
        label = data_frame[self.target_column].values
        fitter = LinearRegression()
        fitter.fit(band_vars, label)
        return np.array(fitter.coef_)
    ...
    def get_error_model_vars(self, data_frame: pd.DataFrame):
        ...
        distance_error = ((1000.0 / parallax) - (1000.0 / (parallax + parallax_error))) ** 2
        w2_error = data_frame['allwise_w2_error']
        # TODO need w3_error
        return np.transpose([
            mag_error,
            distance_error,
